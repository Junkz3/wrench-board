...
description: Lance, surveille, et reporte sur la boucle nocturne `microsolder-evolve`. À utiliser quand l'opérateur dit "monitor evolve", "lance evolve", "status evolve", "où on en est sur evolve", "stop evolve", ou demande un avis sur les commits récents du runner. Mode one-shot via `/monitor-evolve` (résumé immédiat) ou continu via `/loop /monitor-evolve` (auto-paced via ScheduleWakeup, surface uniquement les events notables).
---
...
Tu es l'observateur d'l'opérateur sur la boucle `scripts/evolve-runner.sh`. Tu **ne touches pas le code** que le runner produit (c'est le job de l'agent evolve, skill `microsolder-evolve`). Ton job ici :
...
5. **Stop propre** sur demande d'l'opérateur.
...
| `/loop /monitor-evolve` (continu) | B | Si nouvelles lignes notables → message court à l'opérateur. Sinon silencieux. ScheduleWakeup pour le prochain tick. |
...
Si une vérif rate → message clair à l'opérateur, exit 1, **pas** de ScheduleWakeup.
...
- **Runner down ET l'opérateur n'a rien dit** → mode A: propose-launch (« Le runner est mort depuis $LAST_RUN_AT. Tu veux que je le relance avec `nohup ./scripts/evolve-runner.sh > /tmp/microsolder-evolve.log 2>&1 &` ? »). Mode B: silencieux + reschedule 1800s.
- **Runner down ET l'opérateur a dit "lance"** → lance-le :
  ```bash
  ...
  ```
- **l'opérateur a dit "stop evolve" / "tue le runner"** :
  ```bash
  pkill -f "scripts/evolve-runner.sh"
  sleep 1
  rm -f /tmp/microsolder-evolve.lock
  ```
  Confirme à l'opérateur. Pas de reschedule.
...
Si **aucune** condition → silencieux total (pas de message à l'opérateur).
...
## Commandes textuelles d'l'opérateur à reconnaître

| Phrase d'l'opérateur (FR/EN) | Action |
|---|---|
...
|---|---|
| `evolve/state.json` corrompu (JSON parse fail) | Alert l'opérateur, exit, pas de reschedule |
| Runner crash 3 fois consécutives (3 lignes `crash` adjacentes dans results.tsv) | Alert l'opérateur avec extracts stderr, propose stop |
...
| Stale runner (lockfile présent mais PID mort) | Nettoie : `rm -f /tmp/microsolder-evolve.lock`, alert l'opérateur |
...
- **Tu ne `git revert` jamais.** Si tu détectes un commit gaming, tu **recommandes** à l'opérateur de le revert. La main reste la sienne.
...
Pour une revue plus détaillée (>3 lignes), pointer vers `evolve/reviews/<date>-auto.md` si l'auto-review a tourné, sinon proposer à l'opérateur de lancer une revue manuelle plus poussée via le subagent `code-reviewer`.
...
3. Alert l'opérateur du reset, continue avec un summary mode A complet.
