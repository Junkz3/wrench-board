...
async def test_owner_scoped_write_isolates_tenants(tmp_path: Path, monkeypatch):
    """A report written under owner A must not be visible to owner B, and the
    shared (owner=None) listing must not see either — full tenant isolation."""
    monkeypatch.setenv("MA_MEMORY_STORE_ENABLED", "false")
    ...
    assert (
        list_field_reports(
            device_slug="demo-pi", memory_root=tmp_path, owner_ref="tenant-b"
        )
        == []
    )
    # The shared/self-host scope sees nothing either (the report is tenant-private).
    assert (
        list_field_reports(device_slug="demo-pi", memory_root=tmp_path) == []
    )
    # Owner A sees exactly its own report.
    own = list_field_reports(
        device_slug="demo-pi", memory_root=tmp_path, owner_ref="tenant-a"
    )
    assert len(own) == 1
    assert own[0]["symptom"] == "private A"


async def test_owner_none_uses_legacy_shared_path(tmp_path: Path, monkeypatch):
    """owner_ref=None preserves the pre-existing shared layout (self-host + back-compat)."""
    monkeypatch.setenv("MA_MEMORY_STORE_ENABLED", "false")
    await record_field_report(
        client=None,
        device_slug="demo-pi",
    ...
    assert (tmp_path / "demo-pi" / "field_reports").is_dir()
    assert not (tmp_path / "demo-pi" / "_owners").exists()


async def test_owner_set_skips_shared_ma_mirror(tmp_path: Path, monkeypatch):
    """With an owner bound, the report must NOT be mirrored to the shared device
    MA store (the agent greps it cross-tenant) — mirror is skipped as private."""
    ...
    monkeypatch.setattr(fr_module, "_mirror_to_managed_agents", _boom)
    status = await record_field_report(
        client=MagicMock(),
        device_slug="demo-pi",
        refdes="U7",
    ...
    assert status["ma_mirror_status"] == "skipped:tenant_private"
