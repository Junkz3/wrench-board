# decryptor.py
"""
XZZ container decode module.

Uses an optional Rust acceleration when available for the byte transform.
Falls back to a Python implementation otherwise.
"""
import binascii

# Use the pyca `cryptography` package (already a project dep) instead of
# pycryptodome to avoid pulling another crypto library. TripleDES with an
# 8-byte key is single-DES, identical to DES.MODE_ECB.
from cryptography.hazmat.decrepit.ciphers.algorithms import TripleDES
from cryptography.hazmat.primitives.ciphers import Cipher, modes

_USE_RUST = False


def hex_to_bytes(hex_string: str) -> bytes:
    return binascii.unhexlify(hex_string)


def decrypt_with_des(encrypted_data: bytes, master_key: str) -> bytes:
    """Decode a block with DES (no unpadding — the XZZ format uses none)."""
    key = hex_to_bytes(master_key)
    cipher = Cipher(TripleDES(key), modes.ECB())
    decryptor = cipher.decryptor()
    return decryptor.update(encrypted_data) + decryptor.finalize()


def de_xor_data(data: bytearray, diode_pattern: bytes, logger=None) -> bytearray:
    """
    Apply the XOR byte transform with the key byte at offset 0x10.

    Uses an optional Rust acceleration when available.
    """
    key = data[0x10]
    pos = data.find(diode_pattern)

    if _USE_RUST:
        # Native path
        if pos == -1:
            if logger:
                logger.debug("Diode marker not found, transform applied over the whole file (Rust).")
            return bytearray(_rust_xor(bytes(data), key))  # noqa: F821 — rust ext, only reachable when _USE_RUST
        else:
            if logger:
                logger.info(f"Diode marker found at position: {pos} (Rust)")
            return bytearray(_rust_xor_pattern(bytes(data), key, diode_pattern))  # noqa: F821 — rust ext, only reachable when _USE_RUST
    else:
        # Pure Python fallback
        if pos == -1:
            if logger:
                logger.debug("Diode marker not found, transform applied over the whole file.")
            return bytearray(a ^ key for a in data)
        else:
            if logger:
                logger.info(f"Diode marker found at position: {pos}")
            return bytearray(a ^ key for a in data[:pos]) + data[pos:]


def decrypt_file(data: bytes, master_key: str, diode_pattern: bytes, logger=None) -> bytes:
    """
    Decode XZZ file data (byte transform only; DES is handled per-block).

    Args:
        data: Raw file bytes
        master_key: DES key (not used here, for API compatibility)
        diode_pattern: Pattern marking the end of the transformed region
        logger: Optional logger

    Returns:
        Decoded bytes
    """
    if logger:
        logger.info(f"Decoding file... {'(Rust)' if _USE_RUST else '(Python)'}")

    data_array = bytearray(data)

    if data_array[0x10] != 0x00:
        if logger:
            logger.info("Applying the byte transform...")
        data_array = de_xor_data(data_array, diode_pattern, logger)
    else:
        if logger:
            logger.info("File already decoded (no transform needed).")

    return bytes(data_array)
